...
class PPOAgent:
    ...
    def select_action(self, state, memory):
        with torch.no_grad():
            ...
            memory.states.append(state_tensor.cpu()) 
            memory.actions.append(action.cpu())
            memory.logprobs.append(dist.log_prob(action).cpu())
        return action.item()
...
def main():
    ...
    # --- ИЗМЕНЕНИЕ: Используем прямое значение вместо .n ---
    # Это решает проблему AttributeError, так как мы точно знаем количество действий
    action_dim = 3 
