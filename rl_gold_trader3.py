...
Версия с улучшенной функцией вознаграждения.
"""
...
class NewsTradingEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        # 1. Штраф за совершение сделки (комиссия)
        trade_penalty = -0.1
        ...
        if action == 1 and self.position == 0: # Buy
            ...
            # Награда за закрытие сделки = процент прибыли/убытка
            reward += (profit / (self.entry_price * self.position_size)) * 100 
            self.position = 0
            self.position_size = 0
        
        # 3. Награда/штраф за удержание позиции
        if self.position == 1:
            unrealized_pnl = (current_price - self.entry_price) / self.entry_price
            # Даем небольшую награду за удержание прибыльной позиции
            # и небольшой штраф за удержание убыточной
